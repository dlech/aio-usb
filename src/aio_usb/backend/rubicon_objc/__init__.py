...
class RubiconObjCUsbInPipe(UsbBackendInPipe):
    def __init__(self, pipe: IOUSBHostPipe) -> None:
        self._pipe = pipe
    ...
    async def transfer(self, length: int) -> bytes:
        ...
        def on_complete(status: int, bytesTransferred: int) -> None:
            if status:
                ...
                future.set_result(bytesTransferred)

        def on_complete_(status: int, bytesTransferred: int) -> None:
            # Want to do as little as possible in this callback since it's
            # called on a separate thread and can't handle exceptions properly.
        ...
        error = objc_id()
        data = self._pipe.hostInterface.ioDataWithCapacity(length, error=error)
        if data is None:
            raise NSErrorError(error)

        if not self._pipe.enqueueIORequestWithData(
            data,
            completionTimeout=5,
            error=error,
        ...
        try:
            ...
            if not self._pipe.abortWithOption(
            ...
            try:
                ...
                # abortWithOption() will cause the request to fail with
                # kIOReturnAborted, if it hasn't already completed. This is
...
class RubiconObjCUsbOutPipe(UsbBackendOutPipe):
    def __init__(self, pipe: IOUSBHostPipe) -> None:
        self._pipe = pipe
    ...
    async def transfer(self, data: bytes) -> int:
        ...
        error = objc_id()
        data_ = self._pipe.hostInterface.ioDataWithCapacity(len(data), error=error)
        if data_ is None:
            raise NSErrorError(error)
...
@asynccontextmanager
async def _open_in_pipe(iface: IOUSBHostInterface) -> AsyncGenerator[Any, UsbInPipe]:
    for ep_desc in _iterate_endpoint_descriptors(
        iface.configurationDescriptor,
        iface.interfaceDescriptor,
    ):
        if (ep_desc.contents.bEndpointAddress & 0x80) == 0x80:
            break
    else:
        raise ValueError("No such endpoint for direction in")

    error = objc_id()
    pipe = iface.copyPipeWithAddress(ep_desc.contents.bEndpointAddress, error=error)
    if pipe is None:
        raise NSErrorError(error)

    yield UsbInPipe(RubiconObjCUsbInPipe(pipe))


@asynccontextmanager
async def _open_out_pipe(iface: IOUSBHostInterface) -> AsyncGenerator[Any, UsbOutPipe]:
    for ep_desc in _iterate_endpoint_descriptors(
        iface.configurationDescriptor,
        iface.interfaceDescriptor,
    ):
        if (ep_desc.contents.bEndpointAddress & 0x80) == 0x00:
            break
    else:
        raise ValueError("No such endpoint for direction out")
    ...
    yield UsbOutPipe(RubiconObjCUsbOutPipe(pipe))
...
class RubiconObjCUsbInterface(UsbBackendInterface):
    ...
    def open_in_pipe(self) -> AbstractAsyncContextManager[UsbInPipe]:
        return _open_in_pipe(self._iface)

    @override
    def open_out_pipe(self) -> AbstractAsyncContextManager[UsbOutPipe]:
        return _open_out_pipe(self._iface)
